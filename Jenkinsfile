pipeline {
  ...
  environment {
    ...
    // Cloud (niet gebruikt in deze basis)
...
# --- Edge start/reload (met poort-conflict fix) ---
if ! docker ps --format '{{.Names}}' | grep -q '^sportstore-edge$'; then
  # bestaat niet of draait niet: eerst andere containers die 80/443 publishen stoppen
  block_ports() {
    local p="$1"
    docker ps --filter "publish=${p}" --format '{{.ID}}' | xargs -r docker stop
  }
  block_ports 80
  block_ports 443

  # oude edge weg (als die bestaat maar stopped is)
  ...
  set -e
  if [ $rc -ne 0 ]; then
    echo "ERROR: sportstore-edge kon niet starten op 80/443."
    echo "Diagnose (processen die 80/443 gebruiken):"
    (command -v ss >/dev/null && sudo ss -ltnp | egrep ':80 |:443 ' ) || true
    echo "Containers die poorten publiceren:"
    docker ps --format '{{.ID}}\t{{.Names}}\t{{.Ports}}'
    exit 1
  fi
else
  # draait al -> alleen reload (zero-downtime)
